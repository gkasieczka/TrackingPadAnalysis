...
for rn, r in RunInfo.runs.items():
    ...
    if not infos[dia].has_key(key):
        infos[dia][key] = 0
    infos[dia][key] += 1

keys = []
for dia in infos:
    keys.extend(infos[dia].keys())
keys = sorted(list(set(keys)))

length = max([len(dia) for dia in infos])
string = ''
i = 0

# string = ('%'+'%d'%length+'s;')%'Dia'
string = 'Dia;'
for key in sorted(keys):
            # string += ('%'+'%d'%len(key)+'s;')%key
            string += '%s;'%key
print string
# print '-'*len(string)
for dia in sorted(infos):
    # string = ('%'+'%d'%length+'s;')%dia
    string = '%s;'%dia
    for key in sorted(keys):
        if key in infos[dia]:
            # string += ('%'+'%d'%len(key)+'s')%infos[dia][key]
            string += '%s;'%infos[dia][key]
        else:
            # string += ('%'+'%d'%len(key)+'s;')%'NAN'
            string += ';'
    print string
